...
CREATE or ALTER procedure [dbo].[ShpSld_New_UPDATE]
...
where (pm.ItemCode in ('00000000000010058656','00000000000010058657','00000000000010058658'  --Misc Used not in the BaseInv table (MediaPlayer,Tablet,Phone)
		...
		or ltrim(rtrim(pm.ProductType)) in ('HPBP','PRMO','SUP','HBF','PBF','TRF'))
...
;with xfersF as(
	...
	from ReportsData..SipsTransferBinHeader as h 
        ...
        left  join #excludes e on d.DipsItemCode = e.ItemCode
...
select     
	isnull(xf.minDt,xt.minDt)[minDt]
...
;with MbyPbyS as(
    select md.Yr, md.Wk, md.minDt, st.LocNo, s.section
    from #minDts md cross join #stores st cross join #sections s
) 
select mps.Yr,mps.Wk,mps.minDt
	,mps.Section,mps.LocNo
...
from MbyPbyS mps
	left outer join #ship sh on mps.LocNo=sh.LocNum and mps.Section=sh.Section and mps.minDt=sh.minDt
	left outer join #sales sa on mps.LocNo=sa.LocNum and mps.Section=sa.Section and mps.minDt=sa.minDt
	left outer join #receiveds rc on mps.LocNo=rc.LocNum and mps.Section=rc.Section and mps.minDt=rc.minDt
	left outer join #xfers xr on mps.LocNo=xr.Loc and mps.Section=xr.Section and mps.minDt=xr.minDt

select count(*) from #LocSchemeDetails
...
----Transaction 1--------------------------------
--Add good rows to ShpSld_NEW--------------------[
-----------------------------------------------[
BEGIN TRY
	begin transaction

	insert into ReportsView.dbo.ShpSld_NEW
	select lsd.*
	from #LocSchemeDetails lsd 
		left join ReportsView.dbo.ShpSld_NEW ss 
		on ss.minDt = lsd.minDt and ss.Section = lsd.Section 
		and ss.LocNo = lsd.LocNo 
